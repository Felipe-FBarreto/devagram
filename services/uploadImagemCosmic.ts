...
const storage = multer.memoryStorage(); // fica salvando as imagem na memoria aplicação e só depois ele faz o upload
...
const uploadImagemCosmic = async (req: any) => {
  if (req?.file?.originalname) {
    if (
      ...
      throw new Error("Extensãoda imagem inválida");
    }
    const media_object = {
